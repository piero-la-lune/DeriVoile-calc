...
class FenRatings : public QMainWindow {
	Q_OBJECT
	
	public:
		explicit FenRatings(
			QWidget *parent = 0, QString fileName = "",
			FenPrincipale *fenPrincipale = 0
		);
		~FenRatings();
	...
	private slots:
		void on_show_currentIndexChanged(int index);
		void on_fichier_choisir_clicked();
		void on_fichier_defaut_clicked();
